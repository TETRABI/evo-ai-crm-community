class Inboxes::UpdateWidgetPreChatCustomFieldsJob < ApplicationJob
  ...
  def perform(account, custom_attribute)
    ...
    Rails.logger.info(
      ...
      "account_id=#{account_id} attribute_key=#{attribute_key} behavior=update_if_exists_skip_if_missing"
    )
    ...
    account.web_widgets.where(pre_chat_form_enabled: true).find_each do |web_widget|
      ...
      Rails.logger.info(
        "[WidgetPreChatCustomFields][update] updated job_id=#{current_job_id} account_id=#{account_id} " \
        "web_widget_id=#{web_widget.id} attribute_key=#{attribute_key}"
      )
    end

    Rails.logger.info(
      "[WidgetPreChatCustomFields][update] finish job_id=#{current_job_id} account_id=#{account_id} attribute_key=#{attribute_key} " \
      "processed_widgets=#{processed_widgets} updated_widgets=#{updated_widgets} skipped_widgets=#{skipped_widgets}"
    )
  rescue StandardError => e
    Rails.logger.error(
      "[WidgetPreChatCustomFields][update] error job_id=#{current_job_id} account_id=#{account_id} attribute_key=#{attribute_key} " \
