module ConversationReplyMailerHelper
  ...
  def google_smtp_settings
    return unless @inbox.email? && @inbox.channel.google?
  ...
  def ms_smtp_settings
    return unless @inbox.email? && @inbox.channel.microsoft?
  ...
  def base_smtp_settings(domain)
    {
      address: domain,
      port: 587,
      user_name: @channel.imap_login.presence || @channel.email,
      password: channel_access_token,
      domain: domain,
      tls: false,
